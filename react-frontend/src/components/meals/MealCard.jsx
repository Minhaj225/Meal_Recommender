...
const MealCard = ({ meal, onRate, onFeedback, showRating = true, showFeedback = false, userEmail }) => {
      ...
      <div className="card-body">
        <div className="flex justify-between items-start">
          <h3 className="card-title text-lg">
            {apiUtils.getCategoryIcon(meal.category)} {meal.mealName}
          </h3>
          {meal.confidence && (
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          ...
          <span className={`badge ${apiUtils.getDietColor(meal.dietaryPreference)}`}>
        ...
        {showRating && (
          <div className="card-actions justify-center mt-4">
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                <input
                  ...
                  className={`mask mask-star-2 ${
                    star <= (rating || meal.userRating || 0) ? 'bg-orange-400' : 'bg-gray-300'
                  }`}
            ...
            {isRating && <span className="loading loading-spinner loading-xs ml-2"></span>}
          </div>
        )}

        {feedback !== null && (
          <div className={`alert ${feedback ? 'alert-success' : 'alert-warning'} mt-2`}>
            <span className="text-xs">
              {feedback ? '👍 Added to favorites!' : '👎 We\'ll improve recommendations'}
            </span>
          </div>
        )}
      </div>
