      ...
      <div className="navbar-start">
        <div className="dropdown">
          ...
          <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
            ...
            <li><Link to="/browse">🔍 Browse Meals</Link></li>
            {isAdmin && <li><Link to="/admin/meals">🛠️ Admin</Link></li>}
          </ul>
        </div>
        <Link to="/" className="btn btn-ghost normal-case text-xl">
          🍽️ AI Meal Recommender
        </Link>
      </div>
        ...
        <div className="dropdown dropdown-end">
          <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
            <div className="w-10 rounded-full bg-primary text-primary-content flex items-center justify-center">
              {isAdmin ? <img src={admin} alt="Admin" className="w-6 h-6" /> : <img src={user} alt="User" className="w-6 h-6" />}
            </div>
          </label>
          <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
            <li><Link to="/profile">📝 Edit Profile</Link></li>
            <li><Link to="/recommendations">🤖 Get Recommendations</Link></li>
            <li><Link to="/browse">🔍 Browse All Meals</Link></li>
            <div className="divider my-1"></div>
            <li><Link to="/admin/login">{isAdmin ? '🛠️ Admin Dashboard' : '🔐 Admin Login'}</Link></li>
            {isAdmin && (
              <li>
                <a 
                  href="#" 
