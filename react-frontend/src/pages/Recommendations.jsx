...
const Recommendations = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.recommendations.length === 0) {
          setError("No recommendations found. Try updating your profile preferences.");
        }
      } else {
  ...
  const handleFeedback = async (mealId, liked) => {
    try {
      ...
      console.log(`Successfully recorded feedback for meal ${mealId}: ${liked ? 'liked' : 'disliked'}`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-base-200 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4">🍽️ Personalized Meal Recommendations</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="form-control md:col-span-2 mt-5.5">
                <label className="label">
                  <span className="label-text font-semibold mx-3">Your Email</span>
                </label>
                <input
                  type="email"
                  value={email}
            ...
            <div className="card-body">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p><strong>Diet:</strong> {student.preferences?.dietaryPreference}</p>
                </div>
                <div>
                  <p><strong>Preferred Cuisines:</strong> {student.preferences?.cuisines?.join(', ') || 'All'}</p>
                  <p><strong>Activity Level:</strong> {student.activityLevel}</p>
                </div>
                <div>
                  <p><strong>Daily Calories:</strong> {student.nutritionGoals?.caloriesPerDay}</p>
                  <p><strong>Daily Protein:</strong> {student.nutritionGoals?.proteinGramsPerDay}g</p>
                </div>
              </div>
              <div className="card-actions justify-end">
                <a href={`/profile?email=${email}`} className="btn btn-sm btn-outline">
                  Edit Profile
                </a>
              </div>
            </div>
        ...
        {/* Loading State */}
        {loading && <LoadingSpinner message="Finding the perfect meals for you..." />}
        ...
        {email && !student && !loading && (
          <div className="alert alert-warning mb-8">
            <div>
              ...
              <span>No profile found for this email. Create your profile first for better recommendations!</span>
            </div>
            <div className="flex-none">
              <a href={`/profile?email=${email}`} className="btn btn-sm">Create Profile</a>
            </div>
          </div>
        )}
          ...
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="text-sm text-gray-600">
                {recommendations.filter(m => m.mlRecommended).length} AI-powered recommendations
              </div>
            </div>
              ...
              <div className="card-body">
                ...
                <div className="alert alert-info mt-4">
                  <div className="text-sm">
                    ...
                    <p>Our AI analyzes your preferences, nutrition goals, and dietary restrictions to find meals you'll love. 
                    Rate meals to improve future recommendations!</p>
                  </div>
                </div>
              </div>
            </div>
