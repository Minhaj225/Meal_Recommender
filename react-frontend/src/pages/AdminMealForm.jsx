  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const mealData = {
        ...
        nutritionScore: form.nutritionScore ? Number(form.nutritionScore) : undefined,
        allergens: form.allergens ? form.allergens.split(",").map(a => a.trim()).filter(a => a) : [],
        ingredients: form.ingredients ? form.ingredients.split(",").map(i => i.trim()).filter(i => i) : []
      };
    ...
    <div className="min-h-screen bg-base-100 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-4xl font-bold">🛠️ Admin: Meal Management</h1>
          ...
          <div className="card bg-stone-700 shadow-lg">
            <div className="card-body ">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text mr-3">Allergens (comma-separated)</span>
                  </label>
                  <input
                    name="allergens"
            ...
            <div className="card bg-zinc-700 shadow-lg">
              <div className="card-body">
                ...
                <form onSubmit={handleBulkImport} className="space-y-4">
                  <div className="form-control">
                    <label className="label">
                      <span className="label-text font-semibold text-gray-300 mr-5">CSV File</span>
                    </label>
                    <input
                      type="file"
                      accept=".csv"
              ...
              <div className="card bg-slate-600 shadow-lg text-white">
                <div className="card-body">
                  ...
                  <div className="text-sm">
                    <p className="mb-2">Current ML model uses {mlFeatures.length} features:</p>
                    <div className="flex flex-wrap gap-1">
                      {mlFeatures.map(feature => (
                        <span key={feature} className="badge badge-outline">{feature}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <tbody>
                  {meals.map((meal) => (
                    <tr key={meal._id}>
                      ...
                      <td>
                        <span className={`badge ${meal.cuisine === 'North Indian' ? 'badge-error' : meal.cuisine === 'South Indian' ? 'badge-success' : 'badge-warning'}`}>
                          {meal.cuisine}
                        </span>
                      </td>
                      <td>
                      ...
                      <td>
                        <span className={`badge ${meal.dietaryPreference === 'Vegetarian' ? 'badge-success' : 'badge-error'}`}>
                          {meal.dietaryPreference}
                        </span>
                      </td>
                      <td>
                        {meal.averageRating ? (
                          <span>{meal.averageRating.toFixed(1)} ⭐ ({meal.totalRatings})</span>
                        ) : (
